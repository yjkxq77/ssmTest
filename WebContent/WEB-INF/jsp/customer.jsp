...
<script type="text/javascript">
	...
	*/
	function ckdeletes(){
		...
		$.ajax({
		       type:"POST",
		       url:"<%=basePath%>customer/selectDelect.action",
	...
	*/
    ...
    $(document).ready(function(){  
       $('#btn').click(function(){  
           if(checkData()){  
               $('#form1').ajaxSubmit({    
                   url:'<%=basePath%>customer/ajaxUpload.action',  
                   dataType: 'text',  
                   success: resutlMsg,  
                   error: errorMsg  
               });   
...
<div id="wrapper">
  ...
  <nav class="navbar navbar-default navbar-static-top" role="navigation"
	...
	<div id="page-wrapper">
		...
		<a href="#" class="btn btn-primary" data-toggle="modal" data-target="#newExcelDialog" onclick="">导出</a>
		<a href="#" class="btn btn-primary" data-toggle="modal" data-target="#newExcelDialog" onclick="">导入</a>
		<a href="#" class="btn btn-primary" onclick="ckdeletes()">批量删除</a>
		<div class="row">
			<div class="col-lg-12">
				<div class="panel panel-default">
					...
					<table class="table table-bordered table-striped">
						<thead>
							<tr>
								<th id="ckBox" style="text-align:center;width: 5%;"  data-bind="visible: deleteqx == '1'"> 
                                	<input type="checkbox" id="ckAll" onclick="CKAlls()" />全选
                                </th>
								<th>编号</th>
								<th>客户名称</th>
								<th>客户来源</th>
							...
							<c:forEach items="${page.rows}" var="row">
								<tr>
								    <td style="text-align:center;" data-bind="visible: deleteqx == '1'">
										<div>
											<input type="checkbox" name="subcheck" id="subcheck" value="${row.cust_id}" data-bind="value: id" onclick="CKAllsOne()" />
										</div>
                                    </td>
									<td>${row.cust_id}</td>
									<td>${row.cust_name}</td>
...
<!-- 导出Excel文件模态框 -->
<div class="modal fade" id="newExcelDialog" tabindex="-1" role="dialog"
	aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-header">
	...
	<div class="modal-footer">
		<button type="button" class="btn btn-default" data-dismiss="modal">关闭</button>
		<button type="submit" class="btn btn-primary" onclick="checkData()">上传</button>
		<button type="button" class="btn btn-primary" id="btn" name="btn" >Ajax上传</button>
	</div>
	     </div>
	</div>
</div>
